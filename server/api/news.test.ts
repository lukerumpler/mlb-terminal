...
describe("resilient /api/news route", () => {
  it("falls back from every Nitter mirror to the verified ESPN MLB RSS feed", async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(new Response("mirror down", { status: 503 }))
      .mockResolvedValueOnce(new Response("mirror down", { status: 503 }))
      .mockResolvedValueOnce(new Response(rss("ESPN fallback headline"), { status: 200, headers: { "content-type": "application/rss+xml" } }));
    ...
    expect(result.body).toMatchObject({ status: "tier-2", source: "ESPN MLB RSS" });
    expect((result.body as { items: Array<{ text: string; sourceTier: number }> }).items[0]).toMatchObject({
      text: "ESPN fallback headline summary",
      sourceTier: 2,
    });
    expect((result.body as { sourceStatuses: Array<{ key: string; ok: boolean | null; reason: string | null }> }).sourceStatuses).toEqual([
      ...
      expect.objectContaining({ key: "mlb-official", ok: null, reason: "standby" }),
    ]);
  });

  it("serves a warm successful response without making a second upstream call", async () => {
    const fetchMock = vi.fn(async () => new Response(rss("Official MLB headline"), { status: 200, headers: { "content-type": "application/rss+xml" } }));
    vi.stubGlobal("fetch", fetchMock);

    const first = response();
    const second = response();
    await newsHandler(request("/api/news?kind=mlb&n=2", "198.51.100.42"), first);
    await newsHandler(request("/api/news?kind=mlb&n=2", "198.51.100.42"), second);

    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(second.headers["X-News-Cache"]).toBe("HIT");
    expect((second.body as { status: string; freshness: string }).status).toBe("cached");
  ...
  it("serves a stale verified snapshot when every live source fails after expiry", async () => {
    vi.useFakeTimers();
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(rss("Cached official headline"), { status: 200, headers: { "content-type": "application/rss+xml" } }))
      .mockResolvedValue(new Response("busy", { status: 429, headers: { "Retry-After": "8" } }));
    vi.stubGlobal("fetch", fetchMock);

    const seeded = response();
    await newsHandler(request("/api/news?kind=mlb&n=2", "198.51.100.43"), seeded);
    vi.advanceTimersByTime(15 * 60_000 + 1);

    const stale = response();
    await newsHandler(request("/api/news?kind=mlb&n=2", "198.51.100.43"), stale);

    expect(stale.statusCode).toBe(200);
    expect(stale.headers["X-News-Cache"]).toBe("STALE");
    expect(stale.body).toMatchObject({ status: "cached-fallback", freshness: "stale-cached" });
    expect((stale.body as { items: Array<{ text: string }> }).items[0].text).toBe("Cached official headline summary");

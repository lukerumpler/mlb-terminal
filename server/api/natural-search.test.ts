...
describe("natural-language search handler", () => {
  ...
  it("returns a structured verified destination and reuses the short cache", async () => {
    vi.mocked(invokeLLM).mockResolvedValueOnce({
      choices: [{ message: { content: JSON.stringify({ intent: "team", tab: "overview", entity: "Los Angeles Dodgers", metric: "WAR", interpretation: "Open the verified Dodgers team overview." }) } }],
    } as never);
    const req = { method: "POST", body: { query: "Dodgers team WAR" } } as never;
    const first = response();
    await naturalSearch(req, first as never);
    const second = response();
    await naturalSearch(req, second as never);
    expect(first.statusCode).toBe(200);
    expect(first.body).toMatchObject({ intent: "team", tab: "overview", entity: "Los Angeles Dodgers", metric: "WAR", generated: true });
  ...
  it("returns a safe explicit fallback when the AI provider fails", async () => {
    ...
    const res = response();
    await naturalSearch({ method: "POST", body: { query: "some unclear baseball request" } } as never, res as never);
    expect(res.statusCode).toBe(200);
    expect(res.headers["X-Search-Status"]).toBe("unavailable");
    expect(res.body).toMatchObject({ intent: "unknown", tab: null, entity: null, generated: false });
  });

  it("rejects non-POST requests and empty queries", async () => {
    const methodRes = response();
    await naturalSearch({ method: "GET", body: {} } as never, methodRes as never);
    expect(methodRes.statusCode).toBe(405);
    const queryRes = response();
    await naturalSearch({ method: "POST", body: { query: " " } } as never, queryRes as never);

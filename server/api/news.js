...
const FOX_MLB_RSS = 'https://api.foxsports.com/v2/content/optimized-rss?partnerKey=MB0Wehpmuj2lUhuRhQaafhBjAJqaPU244mlTDK1i&size=30&tags=fs/mlb';
...
function sourcesFor(kind, team, handle = null) {
  if (handle) {
    return [
      ...NITTER_HOSTS.map((host, index) => source(1, `nitter-${index + 1}`, `Nitter mirror ${index + 1}`, `${host}/${handle}/rss`, 'social')),
  ...
  if (team) {
    return [
      source(1, 'mlb-team-official', `MLB.com ${team} club feed`, `https://www.mlb.com/${team}/feeds/news/rss.xml`, 'team'),
  ...
  if (kind === 'college') {
    return [
      source(1, 'ncaa-d1-official', 'NCAA Division I Baseball RSS', NCAA_D1_RSS, 'college'),
      source(2, 'espn-college', 'ESPN college sports RSS', ESPN_COLLEGE_RSS, 'college'),
...
function parseFeed(xml, feedSource, limit) {
  ...
  for (const block of blocks) {
    ...
    items.push({
      ...
      title: title || text,
      url,
      isoDate: parseDate(tagValue(block, ['pubDate', 'dc:date', 'published', 'updated'])),
    });
...
function retryAfterMs(response) {
  ...
  return Number.isFinite(seconds) ? Math.max(0, Math.min(60_000, seconds * 1000)) : 0;
}
...
async function readSource(feedSource, limit) {
  ...
  try {
    ...
    return { ok: false, source: feedSource, reason: error?.name === 'TimeoutError' ? 'timeout' : 'network-error', durationMs: Date.now() - startedAt };
  }
  ...
  try {
    ...
    return { ok: false, source: feedSource, reason: 'body-read-failed', durationMs: Date.now() - startedAt };
  }

  const items = parseFeed(xml, feedSource, limit);
  if (!items.length) {
    return { ok: false, source: feedSource, reason: 'empty-or-unparseable', durationMs: Date.now() - startedAt };
  }

  return { ok: true, source: feedSource, items, durationMs: Date.now() - startedAt };
}
...
function completeSourceStatuses(configuredSources, attempts) {
  return configuredSources.map(feedSource => attempts.find(attempt => attempt.key === feedSource.key) || {
...
export default async function handler(req, res) {
  ...
  if (rawTeam && !team) return res.status(400).json({ error: 'Invalid team slug', items: [] });
  if (rawHandle && !/^[A-Za-z0-9_]{1,50}$/.test(rawHandle)) return res.status(400).json({ error: 'Invalid handle', items: [] });
  const handle = rawHandle || null;
  const limit = Math.min(MAX_ITEMS, Math.max(1, Number.parseInt(url.searchParams.get('n') || '12', 10) || 12));
  const key = cacheKey(handle ? `handle:${handle}` : team ? 'team' : kind, team || handle, limit);
  const now = Date.now();
  ...
  const request = (async () => {
    ...
    if (hit && hit.staleUntil > now) {
      return {
        ...cachePayload(hit, 'cached-fallback', attempts.at(-1)?.reason || 'all-sources-unavailable'),
        sourceStatuses,
        sources: configuredSources,
        attempts,
      };
  ...
  try {
    ...
    res.setHeader('Cache-Control', payload.status === 'unavailable' ? 'no-store' : 'public, s-maxage=900, stale-while-revalidate=3600');
    res.setHeader('X-News-Cache', payload.status === 'cached-fallback' ? 'STALE' : payload.status === 'unavailable' ? 'MISS' : 'MISS');

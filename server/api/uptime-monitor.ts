...
import {
  getUptimeMonitorScheduleByTaskUid,
  listUptimeMonitorChecksSince,
  recordUptimeMonitorCheck,
} from "../db";
...
export async function probeUptimeEndpoint(
  endpoint: string,
  runKey: string,
  checkedAt = new Date(),
  ...
  try {
    const response = await fetchImpl(endpoint, {
      ...
      headers: { "User-Agent": "SKIP-Uptime-Monitor/1.0" },
    });
    ...
    return {
      ...
      latencyMs,
      passed: isPassingUptimeStatus(response.status),
      checkedAt,
      runKey,
    };
  } catch {
...
export async function runUptimeMonitorBatch(
  runKey: string,
  checkedAt = new Date(),
  fetchImpl: typeof fetch = fetch
) {
  const results = await Promise.all(
    UPTIME_MONITOR_ENDPOINTS.map(target =>
      probeUptimeEndpoint(target.url, runKey, checkedAt, fetchImpl)
    )
  );
...
function summaryForTarget(
  target: (typeof UPTIME_MONITOR_ENDPOINTS)[number],
  ...
  return {
    ...
    averageLatencyMs: total
      ? Math.round(matching.reduce((sum, check) => sum + check.latencyMs, 0) / total)
      : null,
...
export async function getUptimeMonitorDashboard(days: 7 | 30) {
  ...
  return {
    ...
    recentByTarget: UPTIME_MONITOR_ENDPOINTS.map(target => ({
      ...target,
      checks: all30DayChecks.filter(check => check.endpoint === target.url).slice(0, 50),
    })),
  };
}
...
export async function scheduledDailyUptimeMonitor(req: Request, res: Response) {
  try {
    ...
    if (!schedule || !schedule.enabled) {
      return res.json({ ok: true, skipped: "orphaned-or-disabled-schedule" });
    }
    const checkedAt = new Date();
    const results = await runUptimeMonitorBatch(
      buildDailyUptimeRunKey(checkedAt),
      checkedAt
    );

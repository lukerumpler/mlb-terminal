...
function pctPopulation(center, spread, n = 60) {
  ...
  for (let i = 0; i < n; i++) {
    const v =
      center + ((Math.sin(i * 12.9898) * 43758.5453) % 1) * spread * 2 - spread;
...
export const mockPitchingSeason = {
...
export const mockPitchingCareer = {
  stats: [{ group: { displayName: "pitching" }, splits: [] }],
};
...
export const mockContract = {
  found: true,
  id: JUDGE_ID,
  name: "Aaron Judge (MOCK TEST DATA)",
  team: "NYY",
  years: "2023-2031",
  totalValue: "$360M",
  aav: "$40M",
...
function savantPopulation(extra = {}) {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    rows.push({
      ...
      est_woba: (0.28 + t * 0.13).toFixed(3),
  ...
  rows.push({
    player_id: JUDGE_ID,
    player_name: "Aaron Judge (MOCK TEST DATA)",
    whiff_percent: "24.1",
    oz_swing_percent: "22.8",
    z_contact_percent: "81.4",
    zone_percent: "46.2",
    groundballs_percent: "38.9",
    est_woba: ".412",
    avg_hit_speed: "95.8",
    launch_angle_avg: "14.2",
    brl_percent: "15.9",
    hard_hit_percent: "54.2",
...
export const mockBatTracking = savantPopulation().map(r => ({
  id: r.player_id,
  ...r,
  avg_bat_speed: "73.5",
  fast_swing_rate: "42.1",
}));

...
export default function RecentHistoryDropdown({ items = [], onClear }) {
  ...
  return (
    <div ref={rootRef} className="skip-history-root" style={{ position:'relative', flexShrink:0 }}>
      <button type="button" className="skip-history-trigger" aria-haspopup="menu" aria-expanded={open} aria-label="Recently viewed players and teams" title="Recently viewed players and teams" onClick={() => setOpen(value => !value)}
      ...
      {open && (
        <div role="menu" aria-label="Recently viewed players and teams" className="skip-history-menu" style={{ position:'absolute', zIndex:30, top:'calc(100% + 8px)', right:0, width:270, maxWidth:'calc(100vw - 24px)', padding:6, border:`1px solid ${C.border}`, borderRadius:10, background:C.surface, boxShadow:`0 16px 32px color-mix(in srgb, ${C.navy} 18%, transparent)` }}>
          ...
          {items.length ? items.map(item => (
            <button key={`${item.type}-${item.type === 'player' ? item.id : item.abbr}`} type="button" role="menuitem" onClick={() => { setOpen(false); item.type === 'player' ? openPlayerProfile(item.id, item.label) : openTeamOverview(item.abbr); }}
              ...
              {item.type === 'team' ? <TeamLogo abbr={item.abbr} size={22} /> : <span aria-hidden="true" style={{ width:22, height:22, display:'grid', placeItems:'center', borderRadius:6, background:C.tealSoft, color:C.teal, ...px({ fontSize:11, fontWeight:900 }) }}>P</span>}
              <span style={{ flex:1, minWidth:0 }}>
                ...
                <span style={sans({ display:'block', marginTop:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', fontSize:9.5, color:C.text3 })}>{item.secondary || (item.type === 'team' ? 'Team overview' : 'Player profile')}</span>
              </span>
              <span style={{ flexShrink:0, ...px({ fontSize:8.5, color:C.text4 }) }}>{formatRecentHistoryTime(item.viewedAt)}</span>
            </button>
          )) : (
            <div style={sans({ padding:'16px 8px', color:C.text3, fontSize:10.5, lineHeight:1.5 })}>Players and teams you open will appear here for quick backtracking.</div>
          )}
        </div>
      )}
    </div>

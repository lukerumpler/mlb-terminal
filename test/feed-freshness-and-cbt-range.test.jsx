...
import {
  CBT_HISTORY_OPTIONS,
  buildCbtHistorySeasons,
  normalizeCbtHistoryRange,
  readCbtHistoryRange,
  saveCbtHistoryRange,
} from "../client/src/lib/cbtHistory.js";
...
describe("feed freshness persistence", () => {
  it("keeps the default visible relative preference and merges partial updates", () => {
    expect(readFeedFreshnessSettings()).toEqual({
      enabled: true,
      displayMode: "relative",
    });
    expect(saveFeedFreshnessSettings({ displayMode: "exact" })).toEqual({
      enabled: true,
      displayMode: "exact",
    });
    expect(saveFeedFreshnessSettings({ enabled: false })).toEqual({
      enabled: false,
      displayMode: "exact",
    });
    expect(readFeedFreshnessSettings()).toEqual({
  ...
  it("records only known successful feeds and preserves their timestamps", () => {
    expect(recordFeedSuccess("mlb-scores", 1_700_000_000_000)).toBe(
      1_700_000_000_000
    );
  ...
  it("uses honest text for missing timestamps and readable relative/exact values", () => {
    ...
    expect(
      formatFeedFreshness(1_700_000_000_000, { now: 1_700_000_120_000 })
    ).toBe("2 mins ago");
    expect(formatFeedFreshness(1_700_000_000_000, { mode: "exact" })).toContain(
      "2023"
    );
  });
  ...
  it("renders each feed and lets the user toggle the global indicator", () => {
    ...
    render(
      <FeedFreshnessPanel
        ...
        successes={{ "mlb-scores": Date.now() - 60_000 }}
    ...
    expect(
      screen.getAllByText("No successful update recorded").length
    ).toBeGreaterThan(0);
    fireEvent.click(
      screen.getByRole("switch", { name: /Toggle data freshness indicator/i })
    );
...
describe("selectable CBT history range", () => {
  ...
  it("builds an inclusive current-season window in chronological order", () => {
    expect(buildCbtHistorySeasons(5, 2026)).toEqual([
      2022, 2023, 2024, 2025, 2026,
    ]);
    expect(buildCbtHistorySeasons(10, 2026)).toEqual([

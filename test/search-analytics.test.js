...
describe('local search analytics', () => {
  it('normalizes query text without retaining long numeric or email-like tokens', () => {
    expect(normalizeSearchQuery('  Juan Soto 2026 OPS  user@example.com  ')).toBe('juan soto ops');
  });

  it('aggregates repeated queries and keeps routing metadata for shortcut prioritization', () => {
    recordSearchQuery('Dodgers team WAR', { intent: 'team', metric: 'WAR', tab: 'overview', status: 'resolved' });
    recordSearchQuery('dodgers team war', { intent: 'team', metric: 'WAR', tab: 'overview', status: 'resolved' });
    recordSearchQuery('Juan Soto OPS', { intent: 'player', metric: 'OPS', tab: 'players', status: 'resolved' });
    const rows = readSearchAnalytics();
    expect(rows[0]).toMatchObject({ query: 'dodgers team war', count: 2, intent: 'team', metric: 'WAR', tab: 'overview', status: 'resolved' });
  ...
  it('clears local analytics without affecting unrelated local storage', () => {
    ...
    recordSearchQuery('Players page', { intent: 'page', tab: 'players', status: 'resolved' });

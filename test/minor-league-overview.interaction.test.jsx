...
const getTeamAffiliates = vi.fn(async teamId =>
  teamId === 137
    ? [
        {
          id: 6141,
...
const getMinorLeagueTeamStandings = vi.fn(async () => ({
  retrievedAt: "2026-08-14T02:00:00.000Z",
  rows: [
    {
      id: 6141,
...
const getMinorLeagueTeamSchedule = vi.fn(async () => ({
  retrievedAt: "2026-08-14T02:00:00.000Z",
  games: [
    {
      gamePk: 1,
      time: "2026-08-15T19:00:00.000Z",
      away: { name: "Sacramento River Cats" },
      home: { name: "Reno Aces" },
      status: "Scheduled",
    },
  ],
}));
const getTeamSavantMetrics = vi.fn(async () => ({
  status: "live",
  retrievedAt: "2026-08-14T02:00:00.000Z",
  expectedWOBA: 0.342,
  exitVelocity: 89.4,
...
const getMinorLeagueTeamOverview = vi.fn(async () => {
  ...
  if (affiliateMode === "loading")
    return new Promise(resolve => {
      resolveAffiliateOverview = () => resolve(affiliateOverview);
    });
...
vi.mock("../client/src/api/mlb.js", async () => {
  ...
  return {
    ...
    getTeamModelSources: vi
      .fn()
      .mockResolvedValue({
        found: false,
        retrievedAt: "2026-08-14T02:00:00.000Z",
        source: "FanGraphs",
        statuses: { playoffOdds: "source-gap", teamWar: "source-gap" },
      }),
    getTeamAffiliates,
...
const { default: OverviewPage } = await import(
...
describe("Team Overview minor-league affiliate interaction", () => {
  ...
  it("selects the Giants and displays the Sacramento River Cats affiliate overview while keeping MLB context visible", async () => {
    ...
    expect(
      await screen.findByText("Minor-League Affiliate Overview")
    ).toBeInTheDocument();
    expect(
      await screen.findByText("Sacramento River Cats")
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Affiliated with San Francisco Giants/)
    ).toBeInTheDocument();
  ...
  it("shows the affiliate loading state before the live overview resolves", async () => {
    ...
    await user.selectOptions(affiliateSelect, "6141");
    expect(
      screen.getByText(/Live MLB identity · stats loading/)
    ).toBeInTheDocument();
  ...
  it("switches between affiliate standings and schedule views and reports Savant freshness", async () => {
    ...
    expect(
      screen.getByText(/Sacramento River Cats @ Reno Aces/)
    ).toBeInTheDocument();
  });

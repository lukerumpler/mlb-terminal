...
import {
  ...
  shouldLoadPlayerVideoThumbnail,
} from "../client/src/pages/PlayersPage.jsx";
import {
  getTeamFinancialRows,
  getRepeaterTierSeverity,
} from "../client/src/pages/PlayersPage.jsx";
...
describe("team financial rows", () => {
  ...
  it("uses increasing visual severity for first-, second-, and third-plus-year tiers", () => {
    ...
    expect(getRepeaterTierSeverity(1).color).not.toBe(
      getRepeaterTierSeverity(3).color
    );
  ...
  it("shows unavailable strings or em dashes when the financial feed is unavailable", () => {
    ...
    expect(
      rows.some(row => row.value === "History unavailable" || row.value === "—")
    ).toBe(true);
  });
});

...
import { buildFarmSystemSummary, scoreFarmSystem } from '../client/src/lib/farmSystemScore.js';
...
describe('farm system summary score', () => {
  ...
  it('orders organizations by the derived score rather than raw count alone', () => {
    const summary = buildFarmSystemSummary({ rankings, trackedCount: 9, poolSize: 100 });
    expect(summary.rows[0].team).toBe('AAA');
    expect(summary.rows[0].metrics.score).toBeGreaterThan(summary.rows[1].metrics.score);
  ...
  it('returns an honest unavailable state when ranking inputs are absent', () => {
    const summary = buildFarmSystemSummary({ rankings: [], trackedCount: 0, poolSize: 0 });

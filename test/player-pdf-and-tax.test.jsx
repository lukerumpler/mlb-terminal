...
import {
  buildMultiYearTaxProjection,
  calculateCbtTax,
  getRepeaterTier,
  getSurchargeBand,
} from "../shared/luxuryTax.js";
import {
  buildExecutiveScoutingSummaryModel,
  buildPlayerValuationCardModel,
} from "../client/src/lib/pdfExports.js";

describe("luxury-tax repeater model", () => {
  it("maps consecutive-year history to MLB repeater tiers", () => {
    expect(getRepeaterTier(1)).toMatchObject({
      label: "First-year CBT payer",
      baseRate: 0.2,
    });
    expect(getRepeaterTier(2)).toMatchObject({
      label: "Second consecutive year",
      baseRate: 0.3,
    });
    expect(getRepeaterTier(3)).toMatchObject({

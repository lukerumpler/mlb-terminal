...
import comparisonSummary, {
  hasValidComparisonPayload,
  fallbackSummary,
} from "../server/api/comparison-summary.js";
...
describe("AI comparison summary contract", () => {
  ...
  it("accepts two same-type players with percentile axes", () => {
    expect(
      hasValidComparisonPayload({ players: [player("A"), player("B")] })
    ).toBe(true);
  });
  ...
  it("rejects incomplete, mismatched, or empty comparison payloads", () => {
    ...
    expect(
      hasValidComparisonPayload({
        players: [player("A"), { ...player("B"), playerType: "pitcher" }],
      })
    ).toBe(false);
    expect(
      hasValidComparisonPayload({
        players: [player("A"), { name: "B", playerType: "hitter", axes: [] }],
      })
    ).toBe(false);
  });
});

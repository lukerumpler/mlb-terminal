...
import PitchShapePanel, {
  aggregatePitcherPitches,
  binVelocities,
} from "../client/src/components/PitchShapePanel.jsx";
...
const MOCK_ARSENAL_PRIMARY = [
  {
    ...
    velocity: 94.0,
    spin_rate: 2104,
    pitcher_break_z_induced: 16.0,
    pitcher_break_x: 11.1,
  ...
  },
  {
    ...
    velocity: 78.8,
    spin_rate: 2183,
    pitcher_break_z_induced: 0.3,
    pitcher_break_x: -6.3,
    release_extension: 6.8,
    whiff_percent: 62.4,
    k_percent: 36.4,
    put_away: 37,
    hard_hit_percent: 15.0,
    xwoba: 0.23,
    run_value: -8,
  },
  {
    ...
    velocity: 83.2,
    spin_rate: 1641,
    pitcher_break_z_induced: 7.0,
    pitcher_break_x: 14.1,
    release_extension: 6.9,
    whiff_percent: 45.3,
    k_percent: 40.9,
    put_away: 60,
    hard_hit_percent: 20.0,
    xwoba: 0.331,
    run_value: -3,
  },
];
...
function buildMockPitches() {
  ...
  for (let i = 0; i < 30; i++)
    pitches.push({
      pitch_type: "FF",
      release_speed: 93 + (i % 4),
      stand: i < 24 ? "R" : "L",
    });
  for (let i = 0; i < 20; i++)
    pitches.push({
      pitch_type: "SL",
      release_speed: 78 + (i % 3),
      stand: i < 2 ? "R" : "L",
    });
  return pitches;
}
...
describe("PitchShapePanel (Roadmap #1)", () => {
  it("shows an empty state, not a crash, when there is no arsenal data", () => {
    ...
    expect(
      screen.getByText(/No Statcast pitch-arsenal data/i)
    ).toBeInTheDocument();
    render(<PitchShapePanel arsenal={[]} />);
    expect(
      screen.getAllByText(/No Statcast pitch-arsenal data/i).length
    ).toBeGreaterThan(0);
  });
  ...
  it("without raw pitch data, keeps the old avg-velocity bar and the honest LHH/RHH caveat (fallback path)", () => {
    ...
    expect(
      screen.queryByText("Velocity Distribution by Pitch")
  ...
  it("with raw pitch data, shows the real velocity histogram and LHH/RHH mini-bars instead", () => {
    render(
      <PitchShapePanel arsenal={MOCK_ARSENAL_PRIMARY} pitches={MOCK_PITCHES} />
    );
    expect(
      screen.getByText("Velocity Distribution by Pitch")
    ).toBeInTheDocument();
    expect(
      screen.queryByText("Avg. Velocity by Pitch")
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(/not split vs\. LHH\/RHH/)
    ).not.toBeInTheDocument();
...
describe("aggregatePitcherPitches (Roadmap #1, pitch-level split)", () => {
  it("groups velocities and LHH/RHH counts per pitch type", () => {
    const { velocityByType, usageByHand } =
      aggregatePitcherPitches(MOCK_PITCHES);
  ...
  it("returns empty structures, not a throw, for null/non-array input", () => {
    expect(aggregatePitcherPitches(null)).toEqual({
      velocityByType: {},
      usageByHand: {},
    });
    expect(aggregatePitcherPitches([])).toEqual({
      velocityByType: {},
      usageByHand: {},
    });
  });

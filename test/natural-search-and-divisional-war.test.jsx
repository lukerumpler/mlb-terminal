...
describe('natural-language search', () => {
  it('shows a structured AI route and opens a verified team overview', async () => {
    ...
    routeNaturalLanguageSearch.mockResolvedValueOnce({
      ...
      metric: 'WAR',
      interpretation: 'Open the verified Dodgers team overview for the requested WAR context.',
      generated: true,
    });
    const opened = vi.fn();
    window.addEventListener('skip-open-team', opened);
    render(<CommandPalette onNavigate={vi.fn()} onOpenProspect={vi.fn()} onClose={vi.fn()} />);
    await user.type(screen.getByRole('combobox', { name: 'Search pages, prospects, or ask SKIP' }), 'Dodgers team WAR');
    await user.click(screen.getByRole('button', { name: 'ASK SKIP' }));
    expect(await screen.findByText('AI ROUTE · VERIFIED DESTINATION')).toBeInTheDocument();
    expect(screen.getByText('Open the verified Dodgers team overview for the requested WAR context.')).toBeInTheDocument();
    await user.click(screen.getByRole('button', { name: /Los Angeles Dodgers/i }));
  ...
  it('resolves a player route through the verified MLB player search before navigation', async () => {
    ...
    render(<CommandPalette onNavigate={vi.fn()} onOpenProspect={vi.fn()} onClose={vi.fn()} />);
    await user.type(screen.getByRole('combobox', { name: 'Search pages, prospects, or ask SKIP' }), 'Juan Soto OPS');
    await user.click(screen.getByRole('button', { name: 'ASK SKIP' }));
    await user.click(await screen.findByRole('button', { name: /Juan Soto/i }));
    await waitFor(() => expect(searchPlayers).toHaveBeenCalledWith('Juan Soto', 5));
    expect(opened).toHaveBeenCalled();
  ...
  it('keeps AI unavailable states explicit instead of inventing a destination', async () => {
    const user = userEvent.setup();
    routeNaturalLanguageSearch.mockRejectedValueOnce(new Error('Natural-language search is unavailable.'));
    render(<CommandPalette onNavigate={vi.fn()} onOpenProspect={vi.fn()} onClose={vi.fn()} />);
...
describe('divisional WAR comparison data and tooltip contract', () => {
  it('returns exact FanGraphs component fields when the aggregate response provides them', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        teams: [
          { team: 'Los Angeles Dodgers', totalWAR: 31.4, battingWAR: 21.2, pitchingWAR: 10.2, defenseWAR: 3.4 },
          { team: 'San Diego Padres', totalWAR: 25.4, battingWAR: 16.4, pitchingWAR: 9.0, defenseWAR: 1.2 },
        ],
      }),
    }));
    const result = await getTeamAggregateWar('Los Angeles Dodgers', ['Los Angeles Dodgers', 'San Diego Padres'], 2026);
    expect(result).toMatchObject({ teamWar: 31.4, offensiveWAR: 21.2, defensiveWAR: 3.4, divisionAverageWAR: 28.4 });
    expect(result.divisionTeams[0]).toMatchObject({ team: 'Los Angeles Dodgers', totalWAR: 31.4, offensiveWAR: 21.2, defensiveWAR: 3.4, pitchingWAR: 10.2 });
  });

  it('keeps the defensive component unavailable when FanGraphs does not return a verified defensive field', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ teams: [{ team: 'Los Angeles Dodgers', totalWAR: 31.4, battingWAR: 21.2, pitchingWAR: 10.2 }] }),
    }));
    const result = await getTeamAggregateWar('Los Angeles Dodgers', ['Los Angeles Dodgers'], 2026);
  ...
  it('contains exact offensive and defensive WAR hover labels and honest unavailable copy', () => {
    ...
    expect(chartSource).toContain('Separate defensive WAR was not returned by the verified FanGraphs aggregate feed.');
  });
});

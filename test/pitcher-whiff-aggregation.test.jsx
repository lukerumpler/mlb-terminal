...
import { weightedWhiffPercent, pitcherWhiffPopulation, weightedArsenalStat, pitcherArsenalStatPopulation } from '../client/src/pages/PlayersPage.jsx';
...
describe('weightedArsenalStat / pitcherArsenalStatPopulation — K% (Roadmap #2, pitcher side)', () => {
  ...
  it('the whiff_percent-fixed wrappers still match calling the generic function directly', () => {
    const rows = [{ pitches: 400, whiff_percent: 33.0 }, { pitches: 100, whiff_percent: 10.0 }];
    expect(weightedWhiffPercent(rows)).toBeCloseTo(weightedArsenalStat(rows, 'whiff_percent'), 8);
  });
});

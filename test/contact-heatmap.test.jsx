...
const rightRow = (x, y) => ({
  intercept_ball_minus_batter_pos_x_inches: x,
  intercept_ball_minus_batter_pos_y_inches: y,
  stand: "R",
});
const leftRow = (x, y) => ({
  intercept_ball_minus_batter_pos_x_inches: x,
...
describe("ContactHeatmap (Roadmap #3)", () => {
  it("shows an empty state, not a crash, when there is no contact-point data", () => {
    ...
    expect(
      screen.getByText(/No Statcast intercept-point data/i)
    ).toBeInTheDocument();
    render(<ContactHeatmap contactPoints={[]} />);
    expect(
      screen.getAllByText(/No Statcast intercept-point data/i).length
    ).toBeGreaterThan(0);
  ...
  it("never fabricates a point when a row is missing either coordinate", () => {
    render(
      <ContactHeatmap
        contactPoints={[
          {
            intercept_ball_minus_batter_pos_x_inches: 5.0,
            intercept_ball_minus_batter_pos_y_inches: null,
            stand: "R",
          },
          {
            intercept_ball_minus_batter_pos_x_inches: null,
            intercept_ball_minus_batter_pos_y_inches: 20.0,
            stand: "R",
          },
        ]}
      />
  ...
  it("ignores rows with non-numeric coordinates rather than crashing", () => {
    render(
      <ContactHeatmap
        contactPoints={[
          {
            intercept_ball_minus_batter_pos_x_inches: "not-a-number",
            intercept_ball_minus_batter_pos_y_inches: 20.0,
            stand: "R",
          },

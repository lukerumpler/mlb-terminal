...
describe("SKIP preserved interactions and persistence", () => {
  ...
  it("persists Watchlist stars across remounts", async () => {
    ...
    expect(screen.getByTitle("Remove from watchlist")).toHaveAttribute(
      "aria-pressed",
      "true"
    );
    expect(screen.getByTestId("watch-count")).toHaveTextContent("1");
  ...
  it("persists Scouting Notes across remounts", async () => {
    ...
    await user.type(
      screen.getByPlaceholderText("Player name"),
      "Test Prospect"
    );
    await user.type(
      screen.getByPlaceholderText("Quick observation…"),
    ...
    expect(stored[0]).toMatchObject({
      ...
      text: "Keep the barrel in the zone.",
    });
    expect(
      screen.getByText("Keep the barrel in the zone.")
    ).toBeInTheDocument();
    first.unmount();
  ...
  it("keeps CompareModal multi-player removal and ProspectCard close behavior", async () => {
    ...
    expect(
      screen.getByRole("dialog", { name: /Compare batters:/i })
    ).toBeInTheDocument();
    expect(screen.getByText("Compare Batters · 4")).toBeInTheDocument();
    await user.click(
      screen.getByRole("button", {
        name: `Remove ${players[0].name} from comparison`,
      })
    );
    ...
    expect(
      screen.getByRole("dialog", { name: /scouting card/i })
    ).toBeInTheDocument();
    await user.click(
      screen.getByRole("button", { name: "Close prospect card" })
    );

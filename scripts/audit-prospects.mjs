...
async function getPerson(record) {
  ...
  try {
    ...
    return {
      ...
      parentOrgId:
        person?.currentTeam?.parentOrgId ?? person?.currentTeam?.id ?? null,
      position: person?.primaryPosition?.abbreviation ?? null,
      birthDate: person?.birthDate ?? null,
      parentOrgAbbr:
        teamById.get(
          person?.currentTeam?.parentOrgId ?? person?.currentTeam?.id
        )?.abbr ?? null,
      nameMatches: person
        ? normalizeName(person.fullName) === normalizeName(record.expectedName)
        : false,
      teamMatches: person
        ? teamById.get(
            person.currentTeam?.parentOrgId ?? person.currentTeam?.id
          )?.abbr === record.expectedTeam
        : false,

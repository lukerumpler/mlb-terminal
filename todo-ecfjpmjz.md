...
- [x] Map the uploaded executive briefing, continuation-link, supporting-analysis handoff, and detailed-card-row density recommendations to current Overview components; the current layout uses `skip-compact-executive-briefing`, has no continuation-link markup, and already contains the named detailed-card decision row.
- [x] Apply all compatible current-layout density fixes without changing data, score logic, source badges, or chart behavior; compact briefing, supporting-analysis handoff, and decision-row rules now use live selectors with content-driven height and compact spacing.
- [x] Add layout regression coverage, validate loaded desktop/tablet/mobile Overview states, and publish the density release; focused/build checks and 648 full tests passed with 3 skips, and desktop, 768px tablet, and 375px mobile screenshots showed no clipping or unintended reserved whitespace.

- [x] Audit the current League and Intelligence workspaces for oversized gaps, inherited minimum heights, panel sizing, and mobile/tablet density inconsistencies; desktop baseline found a legitimate Intelligence comparison-panel height match creating unused space beside Injury Risk, while the League’s visible gap is expected loading skeleton space before comparison data resolves. Post-change review confirmed natural-height panels require an explicit two-column flow to prevent a grid-row gap beneath the comparison controls.
- [x] Apply compatible current-selector spacing and density adjustments to League and Intelligence without changing data, query, or interaction logic; League panel groups now share the 10px Overview rhythm, and Intelligence uses independent stacked columns so its concise comparison panel no longer reserves the Injury panel’s height.
- [x] Add regression coverage, validate desktop/tablet/mobile workspace states, and publish the finalized release to main; focused checks passed, the complete suite passed 651 tests with 3 skips, TypeScript and production build passed, and desktop/mobile browser audits found no horizontal overflow, including the reflowed Intelligence stack.
